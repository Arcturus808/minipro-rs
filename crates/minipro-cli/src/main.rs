...
is omitted and
.B \-r
is specified, the code, data (if applicable), and config (if applicable)
will be written to filename.$ext, filename.eeprom.bin, and filename.fuses.conf
respectively.
If
.B \-c
is omitted and
...
.B \-c config
is specified, no erase is performed to preserve the already programmed data.
