...
- **Man page & completions in GitHub release tarballs** — macOS/Linux CLI releases are now `.tar.gz` archives containing the binary, `man/man1/minipro.1.gz`, and bash/zsh/fish completions (generated by the existing `--generate-man` / `--generate-completions` flags, matching what the GitLab `.deb`/`.rpm` packages already install).
...
- **`--help` footer pointed at unreachable URLs** — the GitLab Pages links advertised in the long help text 404/redirect to sign-in (Pages access control is on and no HTML man page is published). The footer now links to the repo and the GitHub releases page, and points at `--generate-man` for the manual.
