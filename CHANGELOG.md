...
> **Re-release of v0.8.0.** The v0.8.0 GitHub Actions release failed due to a bash quoting bug in the release notes script, compounded by a GitHub Actions outage on Aug 26, 2026. GitLab CLI binaries for v0.8.0 were published successfully. This release re-publishes the same features with the fix. No code changes from v0.8.0.
...
- **Logic IC identify (CLI)** — `--logic-identify --pin-count N` tests an unknown logic IC against all database entries with the matching pin count and prints passing matches. Optional `--vcc` overrides the test voltage. Progress is reported as `Testing candidates... N/M` on stderr. Results are printed as `name  manufacturer` to stdout, matching the SPI autodetect output format. Supported pin counts: 8, 14, 16, 20, 24, 28, 40.

- **Structured logic test grid (GUI)** — the logic test result is now rendered as a zoomable color-coded grid (Ctrl+Scroll to zoom) instead of raw text. Input cells are blue, passing output cells are green, mismatched outputs are red with a `-` suffix, and ignore cells are gray. A copy button exports the full grid as TSV (tab-separated values) for pasting into spreadsheets, word processors, or online posts. The grid uses a header-outside-scroll-container layout to prevent sticky-header overlap.

- **Shared favorites store** — device favorites logic extracted from `DeviceSelector.svelte` to a shared `favorites` writable store in `device.ts`. Both the DeviceSelector and IdentifyResults components read/write the same store, so starring a device from either place updates both UIs immediately. Uses a reactive `$derived` Set for O(1) favorite-name lookups in templates.

- **macOS Gatekeeper workaround documented** — the unsigned `.dmg` triggers macOS Gatekeeper's "'MINIPRO-RS-GUI' is damaged and can't be opened" alert. Documented the `xattr -cr` workaround in README, release notes, and ROADMAP. Code signing and notarization require an Apple Developer Program membership ($99/year) — added to ROADMAP as a future improvement.
...
- **Release notes bash quoting** — fixed a single-quote escaping bug in the GitHub Actions release notes script that prevented the v0.8.0 GitHub release from completing.
...
- **GUI AVR fuse bit decoder** — the config panel now breaks out each AVR fuse/lock byte into individual named bits (CKSEL, SUT, BODLEVEL, SPIEN, RSTDISBL, etc.) with human-readable descriptions and clickable bit cells. Covers all 18 AVR config variants in the database (`avr_1` through `avr_18`), with chip-prefix overrides for configs that span multiple architectures (avr_4, avr_6, avr_13, avr_15, avr_17). Bit definitions sourced from avr-libc device headers and Microchip datasheets. Dangerous bits (RSTDISBL, DWEN, SPIEN, OCDEN, JTAGEN) are highlighted in red. Raw hex input remains visible and stays in sync with bit toggles.
...
- **Write protect integration (CLI + GUI)** — `-u`/`-P` flags and GUI checkboxes now integrate into the write flow matching upstream C minipro. T76 auto-unprotects chips with `off_protect_before` before erase (firmware requirement for parallel NOR). Non-T76 unprotects only if `-u`/"Unprotect before write" checkbox AND the device flag is set. Protect-on after write only if `-P`/"Re-protect after write" checkbox AND `protect_after` flag is set. Warnings printed when a chip may be write-protected. GUI checkboxes are context-specific: visible only when the device has the relevant flag and the programmer model supports protect operations (hidden for T48). "Unprotect before write" is checked by default matching XGPro behavior. Batch mode (CLI `--batch` and GUI batch) also applies protect logic per-chip with the same flag checks and T76 auto-unprotect.
- **GUI logic test vector symbol help** — the logic test options panel now has an info (i) button that opens a help modal explaining the single-character vector symbols used in the test result table (0=Input Low, 1=Input High, L=Output Low, H=Output High, C=Pulse, Z=High-Z, X=Ignore, G=GND, V=VCC). Notes that only L, H, and Z pins are tested; input/power/ground/ignored pins are never marked as errors.
- **SPI flash autodetect (CLI + GUI)** — `-a`/`--spi-autodetect` CLI flag now searches `infoic.xml` for devices with matching JEDEC ID and pin count, printing device names instead of just a raw hex ID. Output matches upstream C minipro format: `Autodetecting device (ID:0xXXXX)` + device names + count. GUI adds "Auto Detect" buttons (8-pin / 16-pin) in the DeviceSelector header — reads JEDEC ID via firmware command 0x37, searches the database, and shows clickable matching device names. Disabled for T56/T76 (protocol support pending). Handles no-chip-detected (ID=0), no-database-match, and multiple-match cases.
- **GUI pin-contact test with ZIF diagram highlighting** — the Diagnostics panel now has a functional "Pin Test" button (previously disabled) that runs the ZIF socket contact test and highlights bad pins on the socket diagram. Bad pin slots render red with "PIN N" labels. A result panel below the diagram shows pass/fail status with a pin list and Clear button. Stale results are cleared on device or programmer change. Supported on TL866II+ and T48 only; disabled on TL866A/CS, T56, and T76. Also disabled in ICSP mode and when no device is selected. Core `Protocol::pin_test()` refactored from `Result<()>` (stderr-only) to `Result<PinTestResult>` with structured `bad_pins: Vec<u16>` — CLI `-z` output format preserved.

### Fixed

- **T76 pin-contact test removed** — the T76 is FPGA-based and the C `minipro` source does not define the bit-banging commands (0x2D-0x36) that the TL866II+/T48 use for contact testing. Matt Brown's t76-improvements branch discovered that the T76's `0x3E` command is an adapter-init pin-driver configuration step, not a standalone contact test — his `t76_adapter_init()` uses it to configure socket pin drivers before bitstream upload. Running it standalone returns meaningless data and can corrupt subsequent reads by disrupting FPGA state. The upstream C minipro's `t76_pin_test` receives the response but never parses it (`value` is initialized to 0 and never updated from the response buffer, so every pin reports as bad). The [xgecu-pro](https://github.com/jfabienke/xgecu-pro) project confirmed on real hardware that it "measured nothing and corrupted every read." T76 now returns `UnsupportedOperation` for pin test, matching TL866A/CS and T56.
