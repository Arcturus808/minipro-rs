...
- [ ] **ZIF socket placement diagram** — visual panel showing the selected device correctly oriented and positioned in the programmer's ZIF socket
  ...
  - **Phase 4 plan: per-class signal-labeled wiring diagrams (scratch-built SVG):**
    ...
    - **Rollout:**
      ...
      - Phase 4b: top five classes (0x09, 0x05, 0x02, 0x0b, 0x0a) — ~95% of ICSP-capable devices — **PARTIAL**: 0x02 PIC (ICD2-compatible: 1=VPP/MCLR 2=VCC 3=GND 4=PGD 5=PGC 6=NC) shipped for TL866A; 0x0b I²C / 0x0a Microwire deferred pending the Xgpro img reference set
      - Phase 4c: AVR variants (0x01, 0x06–0x08) + eMMC (0x40) — **PARTIAL**: 0x01 Atmel SPI ISP and 0x06–0x08 AVR port variants shipped for TL866A as generic signal-name targets (`numbered: false` — chip pin positions vary by package); eMMC still unverified
      - Phase 4d: long tail (0x03, 0x04, 0x0c, 0x0e — ~110 devices combined); may remain as pin-numbering fallback. Note: 0x03/0x04 (SyncMos) have reference images but no current DB devices reference them
  ...
  - **Status:** Phase 1 complete, Phase 2 complete (pin-numbering fallback), Phase 3 not started, Phase 4a complete (verified on TL866A + AT45DB class 0x05), Phase 4b/4c partial (TL866A: 0x01 Atmel SPI, 0x02 PIC ICD2, 0x06/0x07/0x08 AVR variants — verified in GUI + CLI)
